...
const Schedule: React.FC = () => {
    ...
    const filterEvents = () => {
        return events.filter((event: { date: string; location: string; }) => {
            ...
            const matchEventType = !eventTypeFilter || event.location === eventTypeFilter;
            const matchCategory = !categoryFilter || event.location === categoryFilter;
    ...
    return (
        <div className="flex flex-col items-center bg-gradient-to-br from-white to-red-700 min-h-screen p-6">
            <h4 className="text-2xl font-bold text-white mb-8 pt-16">Upcoming Events</h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filterEvents().map((event:any, index:any) => (
                    <div key={index} className="relative border-2 border-red-300 rounded-lg shadow-md overflow-hidden">
                        <div className="w-full h-56 overflow-hidden">
                            <img className="w-full h-full object-cover rounded-t-lg" src={event.imageUrl} alt={event.title} />
                        </div>
                        <div className="p-6 flex flex-col justify-end">
                            ...
                            <p className="text-lg text-gray-300">Location: {event.location}</p>
                            <button className='bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-full transition duration-300 ease-in-out transform hover:scale-105 mt-4'>
                                Register
                            </button>
                        </div>
                    </div>
